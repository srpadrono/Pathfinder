...
The point of TDD is that tests shape the API — code written before its test tends to
produce untestable designs. If you find yourself with implementation code that has no
failing test, delete the implementation and write the test first. The test defines
what "correct" means.
...
For functions, modules, or components that your E2E checkpoints depend on, also write
unit tests. See `pathfinder:unit-testing` for the full unit testing workflow — it covers
naming conventions, co-location, and verification.
...
After writing tests for each checkpoint, update its task file from `planned` to `red`.
See `references/task-tracking.md` for the update procedure and state sync.
