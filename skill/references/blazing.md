...
1. **Read journey map:** Load `$PATHFINDER_DIR/journeys.json`
2. **Generate diagrams:** Run `python3 scripts/generate-diagrams.py $PATHFINDER_DIR/journeys.json`

This creates `$PATHFINDER_DIR/blazes.md` with multiple diagram types:
...
On first run, a baseline is auto-saved to `$PATHFINDER_DIR/journeys-baseline.json`.
...
3. **Commit:** `git add $PATHFINDER_DIR/blazes.md && git commit -m "Diagram: N journeys mapped (X% coverage)"`
